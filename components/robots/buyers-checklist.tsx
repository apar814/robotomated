...
export function BuyersChecklist({ robotName, robotSlug }: BuyersChecklistProps) {
  ...
  return (
    <div className="rounded-md border border-border border-l-2 border-l-lime bg-obsidian-surface p-4">
      ...
      <ol className="mb-3 space-y-2">
        {VISIBLE_QUESTIONS.map((q, i) => (
          <li key={i} className="flex gap-2 text-sm">
            <span className="shrink-0 font-mono text-[11px] font-bold text-lime">
      ...
      {/* Email Form */}
      {status === "success" ? (
        <div className="rounded-md border border-[rgba(200,255,0,0.08)] bg-lime-dim px-3 py-2">
          <p className="font-mono text-sm text-lime">{message}</p>
        </div>
      ) : (
