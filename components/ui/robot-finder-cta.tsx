  ...
  return (
    <section className="border-y border-border bg-gradient-to-r from-[#0A0F1E] via-[#0F1628] to-[#0A0F1E] px-6 py-10">
      <div className="mx-auto flex max-w-4xl flex-col items-center gap-6 text-center sm:flex-row sm:text-left">
        <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-electric-blue/10">
          <svg className="h-7 w-7 text-electric-blue" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
...
export function RobotFinderInlineLink({ industry }: { industry?: string }) {
  ...
  return (
    <Link href={href} className="inline-flex items-center gap-1.5 text-sm font-semibold text-blue hover:underline">
...
export function RobotFinderSidebarCta({ categorySlug }: { categorySlug?: string }) {
  const href = categorySlug
    ...
    : "/tools/robot-finder";
  return (
    <div className="rounded-xl border border-white/[0.07] bg-gradient-to-br from-blue/[0.03] to-violet/[0.03] p-5">
      ...
      <Link
        href={href}
        className="mt-3 inline-block rounded-md bg-blue/10 px-4 py-2 text-xs font-semibold text-blue transition-colors hover:bg-blue/20"
      >
        Take the Quiz &rarr;
      </Link>
    </div>
