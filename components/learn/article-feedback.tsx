...
export function ArticleFeedback({ articleSlug }: { articleSlug: string }) {
  ...
  return (
    <div className="flex items-center gap-4 rounded-xl border border-border bg-navy-light px-6 py-4">
      ...
      {submitted ? (
        <span className="text-sm text-green">Thanks for your feedback!</span>
      ) : (
        <div className="flex gap-2">
          <button
            onClick={() => handleFeedback(true)}
            className="rounded-lg border border-border px-4 py-2 text-sm text-muted transition-colors hover:border-green/30 hover:text-green"
          >
