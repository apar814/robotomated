    ...
    <div>
      ...
      <div className="relative">
        <textarea
          ...
          rows={2}
          className="w-full resize-none rounded-xl border border-white/[0.08] bg-white/[0.03] px-5 py-4 pr-14 font-[family-name:var(--font-ui)] text-[15px] text-white placeholder:text-transparent focus:border-white/30 focus:outline-none focus:ring-0"
          style={{ minHeight: "56px", borderLeftWidth: "3px", borderLeftColor: value ? "rgba(255,255,255,0.3)" : "transparent" }}
        ...
        <button
          ...
          disabled={!value.trim()}
          className="absolute bottom-3 right-3 flex h-8 w-8 items-center justify-center rounded-lg bg-white text-black transition-opacity disabled:opacity-20"
      ...
      <div className="mt-4 flex gap-2 overflow-x-auto pb-1">
        {JOB_CATEGORIES.map((cat) => (
          <button
            ...
            className="flex shrink-0 items-center gap-1.5 rounded-full border border-white/[0.06] px-3 py-1.5 text-[11px] font-medium text-white/40 transition-all hover:border-white/20 hover:text-white/70"
          >
            <span className="text-white/45 transition-colors group-hover:text-white/70">{cat.icon}</span>
            {cat.label}
          </button>
        ))}
      </div>
